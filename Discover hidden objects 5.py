...
def process_tile(rows: list, model, device, session) -> list:
    ...
    log(f"    {len(variable)} variable candidates; cross-matching "
        f"against AllWISE ({KNOWN_OBJECT_WORKERS} concurrent)...")

    # ---- Pass 3: cross-match every surviving candidate against AllWISE
    # concurrently instead of one blocking round-trip at a time. Each
    # candidate's centroid is independent, so this is safe to parallelize. ----
