...
# How many is_known_object() cross-match calls to run concurrently, and how
# many tiles' TAP queries to have in flight at once (current tile being
# processed + next tile's query prefetching in the background).
KNOWN_OBJECT_WORKERS = 12
TILE_PREFETCH_DEPTH = 1
...
def submit_async_job(session: requests.Session, adql: str,
                      extra_data: dict = None, files: dict = None) -> str:
    data = {
    ...
    resp = session.post(TAP_ASYNC_URL, data=data, files=files,
...
def is_known_batch(session, candidates: list):
    """Attempts ONE crossmatch query for ALL candidates in the tile against
    AllWISE, instead of one round-trip per candidate - this is what actually
    scales when a tile has tens of thousands of variable candidates, since
    even a thread pool of one-at-a-time queries is still bounded by total
    request count.

    Uses the standard TAP table-upload feature, submitted as an ASYNC job
    (same submit/poll/fetch/retry path already proven reliable for tile
    queries) rather than a sync request - sync TAP endpoints commonly cap
    execution time and are a poor fit for a large-upload join against a
    catalog the size of AllWISE.

    Returns a list of True/False per candidate (index-aligned with
    `candidates`), or None if the batch approach fails even after retries -
    the caller MUST fall back to the known-good per-candidate
    is_known_object() path in that case; this must never be the only way
    known/new status gets determined.
    """
    if not candidates:
        return []
    ...
    csv_writer.writerow(["cand_id", "ra", "dec"])
    for i, c in enumerate(candidates):
        csv_writer.writerow([i, c["ra_centroid"], c["dec_centroid"]])
    csv_bytes = buf.getvalue().encode("utf-8")

    adql = f"""
        SELECT DISTINCT u.cand_id
        FROM TAP_UPLOAD.cand AS u, {ALLWISE_TABLE} AS a
        WHERE CONTAINS(
    ...
    def do_query():
        job_url = submit_async_job(
            session, adql,
            extra_data={"UPLOAD": "cand,param:cand"},
            files={"cand": ("cand.csv", csv_bytes, "text/csv")},
        )
        phase = poll_job(session, job_url, "allwise_batch_crossmatch")
        if phase != "COMPLETED":
            raise RuntimeError(f"crossmatch job ended in phase {phase}")
        return fetch_job_results(session, job_url)

    try:
        matched_rows = query_with_retry(session, do_query,
                                         "allwise_batch_crossmatch")
        matched_ids = {int(r["cand_id"]) for r in matched_rows}
        return [i in matched_ids for i in range(len(candidates))]
    except Exception as e:
        log(f"    batch crossmatch failed after retries ({e}) - falling "
            f"back to one-by-one AllWISE checks for this tile")
        return None


def build_channels(mjd: np.ndarray, mag: np.ndarray) -> np.ndarray:
    """Builds the same [raw, NDFT, wavelet] feature stack used in
    training. Pure numpy - no GPU/model involved, so this is cheap and
    safe to call once per cluster."""
...
def classify_batch(model, device, channel_stack: list) -> list:
    """Runs ALL clusters from a tile through the classifier in a single
    forward pass instead of one GPU call per cluster. channel_stack is a
    list of arrays as returned by build_channels(). Returns a list of
    {class: prob} dicts, same order as the input."""
    if not channel_stack:
        return []
    x = torch.from_numpy(np.stack(channel_stack)).to(device)
    with torch.no_grad():
        logits = model(x)
        probs = torch.softmax(logits, dim=1).cpu().numpy()
    return [{CLASSES[i]: float(p[i]) for i in range(len(CLASSES))} for p in probs]
...
def process_tile(rows: list, model, device, session) -> list:
    ...
    log(f"    clustering {len(rows)} apparitions with DBSCAN...")
    ...
    # ---- Pass 1: build every cluster's light curve + feature stack. This
    # is pure numpy/CPU work, no network/GPU calls yet, so it's cheap to
    # do for every cluster before deciding what to keep. ----
    #
    # Group row indices by cluster label with ONE sort, not a per-cluster
    # rescan of all rows. On a 3M-row tile with many thousands of clusters,
    # rescanning all rows for every cluster is O(n * num_clusters) and can
    # take hours; this is O(n log n) regardless of cluster count.
    labels = np.asarray(labels)
    order = np.argsort(labels, kind="stable")
    sorted_labels = labels[order]
    unique_labels, start_positions = np.unique(sorted_labels, return_index=True)
    index_groups = np.split(order, start_positions[1:])
    label_to_indices = dict(zip(unique_labels.tolist(), index_groups))
    n_clusters = len(label_to_indices) - (1 if -1 in label_to_indices else 0)
    log(f"    DBSCAN found {n_clusters} candidate clusters "
        f"(+ noise); building light curves...")

    clusters = []
    for cluster_id, idx_arr in label_to_indices.items():
        if cluster_id == -1:
            continue  # noise, not a real source

        cluster_rows = [rows[i] for i in idx_arr]
        ...
        clusters.append({
            ...
            "ra_centroid": float(ras.mean()),
            "dec_centroid": float(decs.mean()),
            "channels": build_channels(mjds_arr, mags_arr),
        })

    if not clusters:
        return []

    log(f"    {len(clusters)} clusters passed min-apparitions filter; "
        f"classifying...")

    # ---- Pass 2: ONE batched forward pass through the classifier for
    # every cluster in the tile, instead of one GPU call per cluster. ----
    all_probs = classify_batch(model, device, [c["channels"] for c in clusters])

    variable = []
    for c, probs in zip(clusters, all_probs):
        top_class = max(probs, key=probs.get)
        if top_class == "null":
            continue  # discard non-variable, matches Paz et al. 2026
        c["predicted_class"] = top_class
        c["probs"] = probs
        variable.append(c)
    ...
    log(f"    {len(variable)} variable candidates; cross-matching "
        f"against AllWISE...")

    # ---- Pass 3: cross-match every surviving candidate against AllWISE.
    # Try ONE batched query for the whole tile first (scales regardless of
    # candidate count); only fall back to one-at-a-time concurrent checks
    # if the batch approach fails. ----
    batch_result = is_known_batch(session, variable)
    if batch_result is not None:
        log(f"    batch crossmatch succeeded ({len(variable)} candidates "
            f"in one query)")
        results = ["known" if is_known else "new" for is_known in batch_result]
    else:
        results = [None] * len(variable)
        with ThreadPoolExecutor(max_workers=KNOWN_OBJECT_WORKERS) as pool:
            future_to_idx = {
                pool.submit(is_known_object, session, c["ra_centroid"], c["dec_centroid"]): i
                for i, c in enumerate(variable)
            }
            for future in as_completed(future_to_idx):
                i = future_to_idx[future]
                known = future.result()
                results[i] = "known" if known else ("new" if known is False else "inconclusive")

    output = []
    for c, known_status in zip(variable, results):
        probs = c["probs"]
        output.append({
            "ra_centroid": c["ra_centroid"],
            "dec_centroid": c["dec_centroid"],
...
def run(tiles, region_name, model_path, checkpoint_path, results_path):
    ...
    # Default pool is only 10 connections - too small once we're firing
    # KNOWN_OBJECT_WORKERS concurrent cross-match requests at IRSA.
    adapter = requests.adapters.HTTPAdapter(
        pool_connections=KNOWN_OBJECT_WORKERS + 8,
        pool_maxsize=KNOWN_OBJECT_WORKERS + 8,
    )
    ...
    pending = [t for t in tiles if t[4] not in state["done_pages"]]
    prefetch_pool = ThreadPoolExecutor(max_workers=TILE_PREFETCH_DEPTH)

    def submit_query(tile):
        ra_min, ra_max, dec_min, dec_max, page_name = tile
        return prefetch_pool.submit(query_tile_async, session, ra_min, ra_max,
                                     dec_min, dec_max, page_name)

    try:
        next_future = submit_query(pending[0]) if pending else None

        for i, (ra_min, ra_max, dec_min, dec_max, page_name) in enumerate(pending):
            log(f"[{i+1}/{len(pending)}] {page_name}: "
                f"RA [{ra_min:.2f},{ra_max:.2f}] "
                f"Dec [{dec_min:.3f},{dec_max:.3f}]")

            # Kick off the NEXT tile's query now, in the background, so its
            # IRSA queue/poll wait overlaps with this tile's DBSCAN/classify/
            # crossmatch work below instead of happening after it.
            this_future = next_future
            if i + 1 < len(pending):
                next_future = submit_query(pending[i + 1])
            else:
                next_future = None

            try:
                rows = this_future.result()
            except Exception as e:
